...
public:
    ...
    {
        graph_[from].push_back({ to, capacity, 0 });
        graph_[to].push_back({ from, 0, 0 }); // Reverse edge with initial flow 0
    }
                ...
                if (parent[edge.to] == -1 && edge.capacity > edge.flow)
                {
                    ...
                    if (edge.to == sink)
                    {
                        ...
                        while (node != source)
                        {
                            ...
                            for (Edge& back_edge : graph_[previous_node])
                            {
                            ...
                            for (Edge& forward_edge : graph_[node])
                            {
...
int main()
{
    ...
    int maximum_flow = network.FordFulkerson(source, sink);
